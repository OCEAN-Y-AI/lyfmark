...
- `npm install` ist im normalen Endkunden-Installer verpflichtend, nicht optional; bei längerer Stille muss eine Wartemeldung erscheinen statt einer verdeckten oder vermuteten Eingabeaufforderung.
- Die LyfMark-VS-Code-Extension-Installation muss die mitgelieferte `.vsix` verwenden und auf Windows direkt `Code.exe` finden können; der Kunden-Installer darf dafür nicht spontan `npx`/`vsce` ausführen.
- Installer-E2E- und manuelle Bootstrap-Tests immer mit temporärem `HOME`/`USERPROFILE` ausführen, damit echte Git-Identität und SSH-Keys des Entwicklers nicht verändert oder ausgelesen werden.
