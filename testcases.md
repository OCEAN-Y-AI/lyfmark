...
- Installer tests must guard that `npm install` is a mandatory non-interactive setup step with visible waiting feedback and that the VS Code extension installer uses the bundled VSIX plus a direct `Code.exe` fallback instead of running `npx` during customer setup.
